...
| `relay_join_attempt` / `relay_join_voice_timeout` / `relay_join_master_property_wait` / `relay_join_master_property_found` / `relay_join_master_property_missing` / `relay_join_calling` / `relay_join_succeeded` / `relay_join_failed` | Internet client flow | Joiner polls the master client player's custom properties for `rc`. `relay_join_master_property_wait` opens the poll with the configured `timeout_seconds`; `_found` fires on success; `_missing` fires on timeout (still 8 s) and includes `waited_seconds`. |
