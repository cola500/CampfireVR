...
| `relay_host_ready` / `relay_host_voice_failed` / `relay_host_player_property_set_failed` | Host pipeline complete or failed | `ready` only fires after verify succeeded; `player_property_set_failed` is the explicit "rc never synced" path so the host surfaces a visible failure instead of silently looking ready while the joiner can't see it |
| `relay_join_attempt` / `relay_join_voice_timeout` / `relay_join_calling` / `relay_join_succeeded` / `relay_join_failed` | Internet client flow | Specific failure points along Relay join. |
| `relay_host_event_broadcast` | Host's `OnPlayerEnteredRoom` callback fired and pushed `rc` to the new actor via Photon `OpRaiseEvent` | Captures `target_actor` + `queued` boolean — unsolicited push path. Plan C1. |
| `relay_code_request_sent` / `relay_code_request_received` | Joiner sends a `RelayCodeRequest` event on room entry; host receives it | Defensive backup against awkward `OnPlayerEnteredRoom` timing. `_received` includes `from_actor`. |
| `relay_code_response_sent` / `relay_code_response_received` | Host responds to a request with the `rc`; joiner consumes | `_sent` includes `target_actor` + `queued`; `_received` includes `code_length`. |
| `relay_code_broadcast_received` | Joiner consumed the host's unsolicited broadcast (from `OnPlayerEnteredRoom`) | First-event-wins between broadcast and response — late one is ignored. |
| `relay_join_code_event_timeout` | Joiner's `WaitForRelayCodeEventAsync` timed out | Includes `timeout_seconds`. Means neither broadcast nor response arrived. |
| `relay_host_voice_offline` / `relay_join_voice_offline` | Axis A preflight detected Photon Voice not at `ConnectedToMasterServer` when user pressed host/join. Triggers one reconnect attempt + 5 s wait before proceeding | Includes `state` (the Photon `ClientState` at the moment of preflight). Means the user pressed at a moment Photon wasn't ready — usually after focus loss / sensor toggle. |
| `relay_host_voice_reconnect_failed` / `relay_join_voice_reconnect_failed` | Preflight reconnect didn't reach `ConnectedToMasterServer` within 5 s | Same `state` field. Surfaces "Voice offline — try again" in-VR; host/join aborts cleanly. |
| `voice_reconnect_attempt` | `VoiceBootstrap.TryReconnect()` issued `ConnectUsingSettings()` | Includes `from_state`. Triggered by preflight, focus-regain, or idle-disconnect path. |
| `voice_reconnect_succeeded` | Photon transitioned to `ConnectedToMasterServer` / `Joined` after a `_attempt` was in flight | Reset path observable in logs — pairs with the preceding `_attempt`. |
| `voice_reconnect_failed` | Photon fell back to `Disconnected` from a "connecting" state while `_attempt` was in flight | Includes `from_state` (the connecting state we fell back from). |
| `voice_focus_regain_reconnect` | `Application.isFocused` flipped false → true and Photon was `Disconnected` at that moment | Includes `from_state`. One-shot per focus edge; user must still re-press host/join. |
| `voice_idle_disconnect_reconnect` | Background reconnect fired after `voice_disconnected_while_in_room` + 5 s backoff while app still focused | Single-shot per disconnect, never loops. Guarded by `_reconnectScheduled`. |
| `ngo_transport_failure_detected` | `NetworkManager.OnTransportFailure` callback fired — NGO has shut down due to a fatal transport error (e.g. Unity Relay "allocation needs to be recreated") | Includes `mode` + `was_host`. Surfaces "Connection dropped" in-VR and resets `_busy` / `_hostedAlias`. |
...
| `ngo_shutdown_wait_started` / `ngo_shutdown_wait_completed` / `ngo_shutdown_wait_timeout` | `StopAsync` awaits `NetworkManager.ShutdownInProgress = false` after calling `Shutdown()` with a 2 s bounded timeout. Fixes Unity Multiplayer SDK `SessionException [Error: NetworkSetupFailed]` race on subsequent re-join without force-stopping the process | `_completed` and `_timeout` include `waited_seconds`. Timeout sets `stop_completed_with_errors`. Verified 2026-06-15 22:36 (failure without wait) vs 2026-06-16 08:04 (pass with force-stop). |
