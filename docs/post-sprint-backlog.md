...
category: meta
status: living
last_updated: 2026-06-15 (3 new multiplayer items + 2 UX priority bumps from Henrik+Johan session; rc race promoted to High with planned fix A+B+C)
sections:
  - About this backlog
  - Conventions
...
| **`rc` discovery via player properties (Plan B — currently shipping)** — Photon Voice's `LoadBalancingClient` does not propagate room `CustomProperties` to new joiners. Verified across two iterations: (1) `relay_host_property_verify_succeeded` in commit `702f7ca` showed `rc` in host's local room view, joiner still saw nothing (johan3-20260615-152324 + henrik3-20260615-152454). (2) Experiment A on top — explicit `RoomOptions.CustomRoomProperties` at creation + `CustomRoomPropertiesForLobby = ["rc"]` — confirmed via `relay_room_options_created` event with `initial_property_rc=yes` on host (johan5-20260615-155242 at 15:49:37) and `initial_property_rc=no` on joiner (henrik5-20260615-155422 at 15:51:07); joiner still got `relay_join_property_missing waited_seconds=8.0` at 15:51:17, **90 seconds after** the host created the room with `rc` embedded. Experiment A definitively failed. **Plan B (shipping)**: pivot to **host player custom properties**. Host writes `rc` on its own `LocalPlayer.CustomProperties`; joiner reads `room.GetPlayer(MasterClientId).CustomProperties["rc"]`. Player properties broadcast unconditionally to all room members at join time and on every update — documented robust path for this class of problem. Renamed log events: `relay_host_player_property_set_attempt/result/verify_*/set_failed` (host), `relay_join_master_property_wait/found/missing` (joiner). All Experiment A code (JoinRoomAsHost, CustomRoomPropertiesForLobby, relay_room_options_created) removed. Existing set + retry + read-back-verify pattern from commit `702f7ca` retained, just operating on player properties instead of room properties. **If Plan B also fails**, the architectural fallback is to drop Photon as the discovery channel entirely and pivot to Unity Multiplayer Service's session metadata directly. | **High** | Before next two-headset test | Evidence chain: `henrik2-20260615-145913` + `johan2-20260615-150114` (fire-and-forget bug), `johan3-20260615-152324` + `henrik3-20260615-152454` (post-702f7ca, host verify worked but joiner still failed), `johan5-20260615-155242` + `henrik5-20260615-155422` (Experiment A applied, definitively failed). |
