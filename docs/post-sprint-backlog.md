...
| **`relay_join_property_missing` race** — joiner repeatedly fails because the host's `SetRoomProperty("rc", relayCode)` call is fire-and-forget and not verified before `relay_host_ready` is logged. **Reproduced cleanly in 2026-06-15 second session**: Johan hosted Relay Room A at 14:53:36 (`relay_host_ready` at 14:53:42), continuously held the host slot until 14:56:47. Henrik joined Photon voice room A at 14:55:58 — definitely the same room as Johan was in — and polled for `rc` for 5 s, returned empty. Three independent join attempts (14:55:57, 14:56:14, 14:57:15) all failed identically. The host's voice room joined fine; the `rc` write never made it through to a state visible on Photon's server. Hypothesis: `room.SetCustomProperties(props)` returned `true` (= queued locally) but the server-side ack never came, or `_voice.Client.CurrentRoom` was momentarily null between voice_joined and the property write. The code currently logs `relay_host_ready` regardless. **Planned fix:** (A) log `SetRoomProperty` return value via new `relay_host_property_set_attempt` + `relay_host_property_set_result` events; (B) host reads `rc` back from its own room and only logs `relay_host_ready` after it sees the write — new `relay_host_property_verify_attempt` + `verify_succeeded` / `verify_failed` events; (C) retry up to 3 times with a short delay, and emit `relay_host_property_set_failed` if all retries fail (host's `_state` then surfaces "Host property set failed — friend can't join" instead of silently looking ready). Joiner-side: bump `WaitForRoomPropertyAsync` timeout 5 s → 8 s and enrich the `relay_join_property_missing` event with the timeout value. | **High** | Before next two-headset test | Two-headset reproduction with clear smoking gun. Fix lands in `NetworkBootstrap.StartHost` only — no scene / voice / XR / build pipeline changes. Quest-logs `henrik2-20260615-145913` + `johan2-20260615-150114` are the evidence base. |
