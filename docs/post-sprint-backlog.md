...
| **30-min connected soak test** — host + join + voice + 30 minutes of mixed activity, watch for crashes, memory growth, thermal throttling | Medium | Before App Lab | Audit's `UNKNOWN` for VRC.Quest.Functional crash-free criteria. Soak test checklist coming in Slice 6. |
| **Frame-time at 90 Hz during connected session** — perf overlay enabled, watch for missed frames during worst case (head + hand sync + voice + Relay) | Medium | Before App Lab | Audit's `UNKNOWN` for VRC.Quest.Performance. Captured in Slice 5 `docs/performance-checklist.md` once that lands. |
...
| **Onboarding / tutorial polish** — refine the world-space TutorialPanel copy + layout | Low | Before App Lab | Current copy is dev-style ("X host, B join, Y mode"). Re-write in tester-friendly language; consider arrows or icons next to button references. |
| **Room / mode explanation** — clarify "Internet" vs "Same Wi-Fi" to a non-developer reader | Low | Before App Lab | Currently a small line at the panel bottom. Could be a one-time tutorial overlay shown on first launch. |
