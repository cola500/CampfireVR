...
category: meta
status: living
last_updated: 2026-06-16 (LAN mode removed entirely — closed rows 4.101 (LAN trap), 4.107 (product simplification), and §3 mode-explanation; CampfireVR now Internet/Relay-only)
sections:
  - About this backlog
  - Conventions
...
| ~~**Room / mode explanation**~~ — **Closed 2026-06-16** by LAN mode removal: there is no longer "Internet" vs "Same Wi-Fi" to explain. CampfireVR is Internet/Relay-only. The tutorial panel was updated to remove the mode-switch line. See `docs/lan-mode-removal-slice.md`. | ~~High~~ closed | done | Closure via simplification, not via better explanation. |
...
| ~~**LAN default `serverAddress: 127.0.0.1` trap**~~ — **Closed 2026-06-16** by `docs/lan-mode-removal-slice.md`: LAN mode removed entirely as a user-facing path. `serverAddress`, `port`, `Mode.Lan` field and all LAN host/join branches deleted from `NetworkBootstrap`. The trap can no longer be triggered because there is no LAN mode to select. | ~~High~~ closed | done | Original fix option (a) "hide Same Wi-Fi mode entirely" was chosen and landed. |
...
| ~~**Product simplification: remove LAN ("Same Wi-Fi") mode as a user-facing path**~~ — **Closed 2026-06-16** by `docs/lan-mode-removal-slice.md`. Deleted: `Mode` enum entirely, `serverAddress` / `port` fields, `LanRoomName`, `ToggleMode`, `OnLeftSecondary` (Y short-tap), `M`-key handler in Editor, `ConfigureLanTransport`, LAN host/join branches in `StartHost`/`StartClient`, mode display in `OnGUI`, "PRESS Y TO SWITCH MODE" from tutorial panel. Anti-pattern observed: no NAT traversal / mDNS / LAN discovery fallback was added. CampfireVR is now Internet/Relay-only. | ~~Medium~~ closed | done | Net ~125 LOC deleted from `NetworkBootstrap.cs` + tutorial UX cleanup. |
