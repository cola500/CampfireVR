...
- **Subtle idle life on the campfire dog** — new `Assets/Scripts/Environment/DogIdleBehaviour.cs` layers three effects on top of the dog's frozen-idle Animator pose: continuous breathing (±5 mm Y bob at ~4 s/cycle), occasional weight shift (±2° Y rotation every 12–25 s), and occasional attention glance (±8° clamped Y rotation toward `Camera.main` every 15–45 s with a 50 % probability gate). Local-camera attention only — no voice activity detection, no networking, no bone-level animation, no audio. Per-frame cost <0.05 ms on Quest 3. `Tools/Quest Setup/Apply Dog Idle Behaviour` editor menu added for retroactive application without destroying manually-positioned DogCompanion. New event `dog_attention_glance` in `DebugLogger` (typically ~1/min). Root cause investigation found vendor `Dog_001_idle.anim` has `m_LoopTime: 0` — the BlendTree idle plays once on spawn and freezes; we layer subtle life on the root transform instead of modifying the vendor asset (`docs/dog-idle-behavior-slice.md`).
...
- **Android `bundleVersionCode` auto-increments per release build**. New `Assets/Editor/Build/VersionCodeGuard.cs` (`IPreprocessBuildWithReport` + `IPostprocessBuildWithReport`) reads `CAMPFIREVR_VERSION_CODE` (computed by `scripts/build-quest.sh` as `git rev-list --count HEAD`), applies it to `PlayerSettings.Android.bundleVersionCode` pre-build, and restores the previous value post-build so `ProjectSettings.asset` on disk stays at its committed baseline. The APK manifest gets the bumped value; the committed asset doesn't drift. `build-info.json` gains a `versionCode` integer field; `DebugLogger` stamps it as `build_version_code` on every `app_started` event; `scripts/package-friend-test.sh` surfaces it in the README + RELEASE-NOTES. `docs/release-process.md` documents the full version-identity chain. Required for Meta Horizon Store / App Lab uploads — every uploaded APK must have a strictly-increasing versionCode (App Lab compliance sprint Slice 3).
- **Application lifecycle handling**. New `Assets/Scripts/Lifecycle/AppLifecycle.cs` (boot-on-load, DontDestroyOnLoad singleton) watches `OnApplicationFocus(bool)` and `OnApplicationPause(bool)`. When Meta system menu opens / headset is removed / app loses focus, Photon Voice 2's `Recorder.TransmitEnabled` is set to false — mic audio no longer streams to Photon's cloud. Restored on focus regain. NGO heartbeats and Unity Relay session deliberately left running; the explicit long-press-Y Stop flow remains the way to fully tear down. Four new debug-log events (`app_lifecycle_ready`, `app_focus_lost` / `app_focus_gained` with `voice_transmit_muted`, `app_paused` / `app_resumed`); `docs/debug-logging.md` updated. Closes a privacy risk identified in `docs/privacy-policy-draft.md` (App Lab compliance sprint Slice 4).
- **Quest 3 target flags + Fixed Foveated Rendering**. `OculusSettings.asset`: `TargetQuest3: 0 → 1`, `TargetQuest3S: 0 → 1` correcting drift (project previously declared Quest 2 support only despite README naming Quest 3 primary). `FoveatedRenderingMethod: 0 → 1` enables FFR — typical 10–30 % GPU saving on Quest 3. `TargetQuest2: 1` kept for older-hardware compatibility; `TargetQuestPro: 0` (discontinued device). New `docs/performance-checklist.md` covers perf-overlay enablement, metrics to watch, pass/warn/fail criteria for a 5-min connected session, and a per-item risk table for high-cost scene items (pine trees, oak wind, dog SkinnedMesh, FireLight shadows, grass overdraw). Conservative deferrals documented (`SubsampledLayout`, `SpaceWarp`, `LateLatching`). Frame-time measurement on a Quest still pending (App Lab compliance sprint Slice 5).
- **Soak-test protocol + report helper**. New `docs/soak-test-checklist.md` (~330 lines) defines a repeatable 30-minute two-headset operational test: pre-test setup (same APK + versionCode, charged + headphones, debug-logs verified, mode + room confirmed), 5-min smoke test, then five timed soak blocks (idle conversation, focus-loss stress, stop/rejoin cycle, extended idle, stress test, calm idle), plus failure conditions, indie-realistic pass criteria, log-collection workflow (cross-references `docs/remote-fika-test-debug-checklist.md`), post-test notes template, and a short regression checklist to run between slices. New `scripts/create-soak-test-report.sh` scaffolds `quest-logs/soak-tests/<stamp>/` with a `BUILD-INFO.json` snapshot of the build under test and a `NOTES.md` template pre-filled with the pass-criteria checklist + per-block observation prompts; refuses to overwrite same-minute runs (use `--label` to disambiguate). Closes the sprint's last remaining slice (App Lab compliance sprint Slice 6).
